...
class Status(object):
    ...
    def update_tempreading(self, tempstr):
        ...
        bed_temp = float(
            temps["B"][0]) if "B" in temps and temps["B"][0] else None
...
class Pronsole(cmd.Cmd):
    def __init__(self):
        ...
        self.settings._add(BuildDimensionsSetting("build_dimensions", "200x200x100+0+0+0+0+0+0", ("Build dimensions"), (
            "Dimensions of Build Platform\n & optional offset of origin\n & optional switch position\n\nExamples:\n   XXXxYYY\n   XXX,YYY,ZZZ\n   XXXxYYYxZZZ+OffX+OffY+OffZ\nXXXxYYYxZZZ+OffX+OffY+OffZ+HomeX+HomeY+HomeZ"),
                                                  "Printer"), self.update_build_dimensions)
    ...
    def promptf(self):
        ...
        if "%" not in promptstr:
            ...
            specials["extruder_temp_target"] = str(
                int(self.status.extruder_temp_target))
            specials["port"] = self.settings.port[5:]
            if self.status.extruder_temp_target == 0:
                specials["extruder_temp_fancy"] = str(
                    int(self.status.extruder_temp)) + DEG
            else:
                specials["extruder_temp_fancy"] = "%s%s/%s%s" % (str(int(
                    self.status.extruder_temp)), DEG, str(int(self.status.extruder_temp_target)), DEG)
            if self.p.printing:
                progress = int(1000 * float(self.p.queueindex) /
                               len(self.p.mainqueue)) / 10
...
(this will terminate the print)."))
    ...
    def end_macro(self):
        ...
        if self.cur_macro_def != "":
            ...
            setattr(self.__class__, "do_" + self.cur_macro_name,
                    lambda self, largs, macro=macro: macro(self, *largs.split()))
            setattr(self.__class__, "help_" + self.cur_macro_name, lambda self,
                                                                          macro_name=self.cur_macro_name: self.subhelp_macro(
                macro_name))
    ...
    def subhelp_macro(self, macro_name):
        if macro_name in self.macros.keys():
            ...
            if "\n" in macro_def:
                ...
                self.log("Macro '" + macro_name +
                         "' defined as: '" + macro_def + "'")
        else:
            self.logError("Macro '" + macro_name + "' is not defined")
    ...
    def set(self, var, str):
        try:
            ...
            if hasattr(ve, "from_validator"):
                self.logError(("Bad value %s for variable '%s': %s") % (
                    str, var, ve.args[0]))
            else:
                self.logError(("Bad value for variable '%s', expecting %s (%s)") % (
                    var, repr(t)[1:-1], ve.args[0]))
    ...
    def load_default_rc(self, rc_filename=".pronsolerc"):
        ...
        try:
            try:
                self.load_rc(os.path.join(
                    os.path.expanduser("~"), rc_filename))
            except IOError:
                self.load_rc(rc_filename)
        except IOError:
            # make sure the filename is initialized
            self.rc_filename = os.path.abspath(
                os.path.join(os.path.expanduser("~"), rc_filename))
    ...
    def connect_to_printer(self, port, baud, dtr):
        try:
            ...
            if e.errno == 2:
                ...
                self.logError(
                    ("Error: You don't have permission to open %s.") % port)
                self.logError(
    ...
    def do_connect(self, l):
        ...
        if len(p) == 0 and not port:
            ...
            return
        if len(a) == 0:
            self.log("No port specified - connecting to %s at %dbps" %
                     (port, baud))
    ...
    def _do_load(self, filename):
        ...
        self.log(("Estimated duration: %d layers, %s") %
    ...
    def do_slice(self, l):
        ...
        try:
            if settings:
                ...
                stl_name = l[0]
                gcode_name = stl_name.replace(
                    ".stl", "_export.Gcode").replace(".STL", "_export.Gcode")
                run_command(command,
                            {"$s": stl_name,
                             "$o": gcode_name},
                            blocking=True)
    ...
    def help_slice(self):
        ...
        self.log(
            ("slice filename.stl view - create Gcode file and view using skeiniso (if using skeinforge)"))
    ...
    def do_upload(self, l):
        ...
        try:
            ...
            while self.p.printing:
                time.sleep(0.5)
                sys.stdout.write("\b\b\b\b\b%04.1f%%" % (
                    100 * float(self.p.queueindex) / len(self.p.mainqueue),))
                sys.stdout.flush()
            ...
            self.log(
                ("Upload completed. %s should now be on the card.") % targetname)
            return
            ...
            self.p.cancelprint()
            self.logError(
                ("A partial file named %s may have been written to the sd card.") % targetname)
    ...
    def help_print(self):
        if not self.fgcode:
            ...
            self.log(
                ("Send a loaded Gcode file to the printer. You have %s loaded right now.") % self.filename)
    ...
    def endcb(self):
        try:
            ...
            self.logError(("Failed to set power settings:")
                          + "\n" + traceback.format_exc())
        print_duration = int(
            time.time() - self.starttime + self.extra_print_time)
            ...
            if self.fgcode is not None:
                ...
                if (len(self.fgcode.filament_length_multi) > 1):
                    for i in enumerate(self.fgcode.filament_length_multi):
                        if self.spool_manager.getSpoolName(i[0]) is not None:
                            self.spool_manager.editLength(
                                -i[1], extruder=i[0])
                else:
                    if self.spool_manager.getSpoolName(0) is not None:
                        self.spool_manager.editLength(
    ...
    def recvcb_actions(self, l):
        if l.startswith("!!"):
            self.do_pause(None)
            msg = l.split(" ", 1)
            if len(msg) > 1 and self.silent is False:
                self.logError(msg[1].ljust(15))
            sys.stdout.write(self.promptf())
            sys.stdout.flush()
            return True
        elif l.startswith("//"):
        ...
        if self.sdprinting or self.uploading:
            if self.uploading:
                fractioncomplete = float(
                    self.p.queueindex) / len(self.p.mainqueue)
            else:
            ...
            secondselapsed = int(
                time.time() - self.starttime + self.extra_print_time)
            secondsremain, secondsestimate = self.compute_eta(
                self.p.queueindex, secondselapsed)
    ...
    def set_temp_preset(self, key, value):
        if not key.startswith("bed"):
            ...
            self.log("Hotend temperature presets updated, pla:%s, abs:%s" %
                     (self.temps["pla"], self.temps["abs"]))
        else:
            self.bedtemps["pla"] = str(self.settings.bedtemp_pla)
            self.bedtemps["abs"] = str(self.settings.bedtemp_abs)
            self.log("Bed temperature presets updated, pla:%s, abs:%s" %
                     (self.bedtemps["pla"], self.bedtemps["abs"]))

    def tempcb(self, l):
        if "T:" in l:
            self.log(l.strip().replace("T", "Hotend").replace(
                "B", "Bed").replace("ok ", ""))
    ...
    def do_gettemp(self, l):
        ...
        if self.p.online:
            ...
            if not self.status.bed_enabled:
                self.log(("Hotend: %s%s/%s%s") % (self.status.extruder_temp,
                                                  DEG, self.status.extruder_temp_target, DEG))
            else:
                self.log(("Hotend: %s%s/%s%s") % (self.status.extruder_temp,
                                                  DEG, self.status.extruder_temp_target, DEG))
                self.log(("Bed:    %s%s/%s%s") % (self.status.bed_temp,
                                                  DEG, self.status.bed_temp_target, DEG))
    ...
    def do_settemp(self, l):
        ...
        try:
            ...
            self.logError(("You must enter a temperature."))
            return

        if f >= 0:
            if f > 250:
                self.log(
                    ("%s is a high temperature to set your extruder to. Are you sure you want to do that?") % f)
                if not self.confirm():
                    return
            if self.p.online:
                self.p.send_now("M104 S" + l)
                self.log(
                    ("Setting hotend temperature to %s degrees Celsius.") % f)
            else:
                self.logError(("Printer is not online."))
        else:
            self.logError(
    ...
    def help_settemp(self):
        ...
        self.log(
            ", ".join([i + "(" + self.temps[i] + ")" for i in self.temps.keys()]))
    ...
    def do_bedtemp(self, l):
        ...
        if f is not None and f >= 0:
            if self.p.online:
                ...
                self.log(("Setting bed temperature to %s degrees Celsius.") % f)
            else:
                self.logError(("Printer is not online."))
        else:
            self.logError(
    ...
    def help_bedtemp(self):
        ...
        self.log(
            ("Enter either a temperature in celsius or one of the following keywords"))
        self.log(
            ", ".join([i + "(" + self.bedtemps[i] + ")" for i in self.bedtemps.keys()]))
    ...
    def do_monitor(self, l):
        ...
        if len(l):
            try:
                ...
                self.logError(("Invalid period given."))
        ...
        try:
            while True:
                ...
                if self.p.printing:
                    ...
                    progress = 100 * \
                               float(self.p.queueindex) / len(self.p.mainqueue)
                elif self.sdprinting:
                    preface = ("SD print progress: ")
                    progress = self.percentdone
    ...
    def help_move(self):
        ...
        self.log(("move X 10 will move the X axis forward by 10mm at %s mm/min (default XY speed)") %
                 self.settings.xy_feedrate)
        self.log(
            ("move Y 10 5000 will move the Y axis forward by 10mm at 5000mm/min"))
        self.log(("move Z -1 will move the Z axis down by 1mm at %s mm/min (default Z speed)") %
                 self.settings.z_feedrate)
    ...
    def complete_move(self, text, line, begidx, endidx):
        if (len(line.split()) == 2 and line[-1] != " ") or (len(line.split()) == 1 and line[-1] == " "):
            ...
            if line[-1] == " ":
                base = ""
            return [i[rlen:] for i in
                    ["-100", "-10", "-1", "-0.1", "100", "10", "1", "0.1", "-50", "-5", "-0.5", "50", "5", "0.5",
                     "-200", "-20", "-2", "-0.2", "200", "20", "2", "0.2"] if i.startswith(base)]
        else:
            return []
    ...
    def do_extrude_final(self, length, feed):
        ...
        self.spool_manager.refresh()
        if self.spool_manager.getSpoolName(self.current_tool) is not None:
            self.spool_manager.editLength(-length,
                                          extruder=self.current_tool)

    def help_extrude(self):
        self.log(
            ("Extrudes a length of filament, 5mm by default, or the number of mm given as a parameter"))
    ...
    def help_reverse(self):
        self.log(
            ("Reverses the extruder, 5mm by default, or the number of mm given as a parameter"))
        ...
        self.log(
            ("reverse 10 210 - extrudes 10mm of filament at 210mm/min (3.5mm/s)"))
    ...
    def help_home(self):
        ...
        self.log(
            ("home - homes all axes and zeroes the extruder(Using G28 and G92)"))
    ...
    def help_run_gcode_script(self):
        self.log((
            "Runs a custom script which output Gcode which will in turn be executed. Current Gcode filename can be given using $s token."))
